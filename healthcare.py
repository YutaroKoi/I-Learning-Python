...
from tools import bmi, sqlite2google, sqlite2fmt, getdigest, is_password_ok, obesity_degree, blood_pressure
...
def blood_pressure_api():
    ...
    if 'tqx' in request.query:
        ...
        if tqx:
            req_id = dict([p.split(':')
                           for p in tqx.split(';')]).get('reqId', req_id)
...
def health_blood():
    ...
    if row:
        latest_high_pressure = str(row[2])
        latest_low_pressure = str(row[3])
        # 直近のデータをテンプレートに埋め込むための変数
        params['latest_date'] = sqlite2fmt(row[1], '/')
        params['latest_high_pressure'] = latest_high_pressure
        params['latest_low_pressure'] = latest_low_pressure
        params['latest_pulse'] = str(row[4])
        params['blood_pressure'] = blood_pressure(
            latest_high_pressure, latest_low_pressure)
...
def health_blood_post():
    ...
    db.execute('''INSERT INTO blood_data
			...
			,pulse )
		VALUES(?,?,?,?,?)''',
               (params['account_id'], params['datetime'].replace('/', '-')			, params['high_pressure'], params['low_pressure'], params['pulse'],))
    conn.commit()
    db.close()
...
def health_bmi():
    ...
    params['weight'] = ''
    params['latest_date'] = ''
    params['obesity_degree'] = ''

    for row in rows:
        latest_bmi = str(round(bmi(float(row[2]), float(row[3])),1))
        date = sqlite2google(row[1])
        records = records + \
            "[new Date(" + date + "),25," + latest_bmi + ",22,18.5,],\n\t"
        weight_hist = weight_hist + \
            "[new Date(" + date + ")," + str(row[3]) + ",],\n\t"
        params['bmi'] = str(latest_bmi)
        params['height'] = str(row[2])
        params['weight'] = str(row[3])
        params['latest_date'] = sqlite2fmt(row[1], '/')
        params['obesity_degree'] = obesity_degree(latest_bmi)
...
def health_bmi_post():
    ...
    db.execute('''INSERT INTO bmi_data
			...
			,weight)
		VALUES(?,?,?,?)''',
               (params['account_id'], params['date'].replace('/', '-'), params['height'], params['weight'],))
    conn.commit()
    db.close()
    return redirect('/health-bmi')
...
def password_settings_post():
    ...
    is_ok = is_password_ok(
        params['current_password'], app_session['account_id'], db_params['password'])
    if not is_ok:
        params['status'] = 'error'
        params['error_msg'] = 'パスワードに誤りがあります。'
        return template('./views/password_settings.html', params)
...
def settings_post():
    ...
    db.execute('''UPDATE users
		SET account_name= ?
			...
			,birthday = ?
		WHERE account_id=?''',
               (params['account_name'], params['user_photo'], params['email'], params['serial'], params['birthday'], params['account_id'],))
    conn.commit()
    db.close()
...
def read_settings(account_id):
    ...
    db = sqlite3.connect(FILENAME).cursor()
    row = db.execute('SELECT * from users where account_id=?',
                     (account_id,)).fetchone()
...
def signup_post():
    params = {}
    # 登録処理
    request_params_key = ['account_name',
                          'password', 'email', 'serial', 'birthday']
    # リクエストパラメータの値を取得
    for key in request_params_key:
        params[key] = unicode(request.forms.get(key, ''), 'utf-8')
    # ファイルデータを取得
    ...
    db.execute('''INSERT INTO users
			...
			,birthday )
		VALUES(?,?,?,?,?,?)''',
               (params['account_name'], params['user_photo'], params['password'], params['email'], params['serial'], params['birthday'],))
    conn.commit()
